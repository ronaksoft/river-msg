...
type IBotProxy interface {
	ServeBotRequest(ctx *edge.RequestCtx, req *MessageEnvelope, res *MessageEnvelope)
}
...
func (sw *BotProxyWrapper) ServeBotRequestWrapper(ctx *edge.RequestCtx, in *rony.MessageEnvelope) {
	req := PoolMessageEnvelope.Get()
	defer PoolMessageEnvelope.Put(req)
	res := PoolMessageEnvelope.Get()
	defer PoolMessageEnvelope.Put(res)
	...
	if !ctx.Stopped() {
		ctx.PushMessage(C_MessageEnvelope, res)
	}
}
...
func (c *BotProxyClient) ServeBotRequest(req *MessageEnvelope, kvs ...*rony.KeyValue) (*MessageEnvelope, error) {
